...
# Persist rate-limit usage (+ the active account email) for the VSCode Explorer progress bar
# (claude-status extension polls this). Only when stdin actually carried rate_limits; coerce missing
# percents to JSON null. Email is read from the session's config dir so it reflects the right account
# under multi-account use (CLAUDE_CONFIG_DIR).
if [ "${HAS_RL:-0}" = 1 ]; then
  _n5="$U5"; [[ "$_n5" =~ ^[0-9]+$ ]] || _n5=null
  _n7="$U7"; [[ "$_n7" =~ ^[0-9]+$ ]] || _n7=null
  if [ -n "${CLAUDE_CONFIG_DIR:-}" ]; then _cfg="$CLAUDE_CONFIG_DIR/.claude.json"; else _cfg="$HOME/.claude.json"; fi
  _email=$(jq -r '.oauthAccount.emailAddress // empty' "$_cfg" 2>/dev/null)
  # write into THIS account's own config dir so accounts don't overwrite each other's usage
  _rl="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/rate-limits.json"; _rlt="$_rl.tmp.$$"
  jq -cn --argjson u5 "$_n5" --argjson u7 "$_n7" --argjson r5 "${R5:-0}" --argjson r7 "${R7:-0}" \
        --arg email "$_email" --argjson ts "$(date +%s)" \
    '{five_hour:{used:$u5,resets_at:$r5},seven_day:{used:$u7,resets_at:$r7},email:$email,ts:$ts}' \
    > "$_rlt" 2>/dev/null && mv -f "$_rlt" "$_rl" 2>/dev/null || rm -f "$_rlt" 2>/dev/null
fi
